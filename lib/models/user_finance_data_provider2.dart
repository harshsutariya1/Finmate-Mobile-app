...
class UserFinanceDataNotifier extends _$UserFinanceDataNotifier {
  ...
  Future<UserFinanceData?> getUserFinanceData(String uid) async {
    if (await fetchUserFinanceData(uid)) {
      logger.i("user finance data set: ${state.listOfTransactions}");
      return state;
    } else {
      return null;
    }
  ...
  Future<void> setUserFinanceData(UserFinanceData userFinanceData) async{
    state = userFinanceData;
    logger.i("user finance data set: ${state.listOfTransactions}");
  }
}
